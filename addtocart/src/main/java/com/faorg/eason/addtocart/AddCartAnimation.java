package com.faorg.eason.addtocart;
...
public class AddCartAnimation {
     ...
     */
    public static void AddToCart(final ImageView startView, final View endView, Context mContext, final RelativeLayout rl, final int time,final int zheng) {
...
//        final float xv = XtoX / time;
        ...
        va.setEvaluator(new TypeEvaluator<PointF>() {
            ...
            public PointF evaluate(float v, PointF pointF, PointF t1) {
                ...
                point.x = v * xv * time;
                if (xv>0) {
                    point.y =zheng* (g * (v * time) * (v * time) / 2 - xv * v * time);
                }else {
                    point.y = zheng*(g * (v * time) * (v * time) / 2 + xv * v * time);
