 ...
 */
public class HomeDevicePresenter implements HomeDeviceContract.Presenter, BleDeviceHelp.BleReceiver {
    ...
    @NonNull
    private final HomeDeviceContract.View registerView;
    Context context;
    private DaoCacheManage daoManage;

    private Baby boundedBaby;

    public HomeDevicePresenter(Context contexts, @NonNull HomeDeviceContract.View registerView) {
...
//                        top_view_left_text.setText("已连接");
    ...
    public void getConnState(final int mConnectionState) {
        ((HomeDeviceFragment) registerView).getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {
...
//                        top_view_left_text.setText(getString(R.string.home_connected));
                        registerView.setTopBarLeftText(((HomeDeviceFragment) registerView).getActivity().getString(R.string.home_connected));
...
//                        top_view_left_text.setText(getString(R.string.home_connectting));
                        registerView.setTopBarLeftText(((HomeDeviceFragment) registerView).getActivity().getString(R.string.home_connectting));
                        break;
                    default:
                        DisConnectState();
                }
    ...
    public void getCmdData(byte[] characteristic) {
        ...
        switch (id) {
            case ICmdModel.CmRxBatteryInfo.ID:
                ...
                registerView.setBatteryInfoDisplay(((HomeDeviceFragment) registerView).getActivity().getString(R.string.home_battery), mBatteryInfo.toString(), 0);
                Setting.getInstance(((HomeDeviceFragment) registerView).getActivity()).setBattery(mBatteryInfo.toString());
//                ((HomeDeviceFragment)registerView).getActivity().runOnUiThread(new Runnable() {
//                    @Override
//                    public void run() {
...
//                });
            ...
            case ICmdModel.CmRxTemperature.ID:
                ...
                if (Utils.isNumber(mTemperature.toString())) {
                    ...
                    //闹铃
                    if (Setting.getInstance(((HomeDeviceFragment) registerView).getActivity()).IsAlarm()) {
                        if (isAlarm) {
                            if (Float.valueOf(mTemperature.toString()) > Float.valueOf(Setting.getInstance(((HomeDeviceFragment) registerView).getActivity()).getTemp())) {//报警字体显示的是红色
                                //计算十分钟之后闹钟继续闹
                                AwayTime = new TimeAway(1800000, 1000);// 构造CountDownTimer对象
                                AwayTime.start();

                                isAlarm = false;
                                Intent intent = new Intent(((HomeDeviceFragment) registerView).getActivity(), ShowAlarm.class);
                                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                                intent.putExtra(UtilKey.ALARM_KEY_ALARM, "0");
                                ((HomeDeviceFragment) registerView).getActivity().startActivity(intent);

                            }
                        }
                    }
                    if (Float.valueOf(mTemperature.toString()) > 35.9 && Float.valueOf(mTemperature.toString()) < 37.5) {
                        registerView.setTemperatureStateDisplay(((HomeDeviceFragment) registerView).getActivity().getString(R.string.home_bottom), R.color.them_bg);
                        ...
                        registerView.setTemperatureStateDisplay(((HomeDeviceFragment) registerView).getActivity().getString(R.string.home_state_low), R.color.them_text);
                        ...
                        registerView.setTemperatureStateDisplay(((HomeDeviceFragment) registerView).getActivity().getString(R.string.home_state_high), R.color.red_light);
    ...
    public void isDistantDisconnect() {
        ((HomeDeviceFragment) registerView).getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (Setting.getInstance(((HomeDeviceFragment) registerView).getActivity()).IsLose()) {//防丢功能已经开启
                    if (isAway) {//防丢功能没有处理
                        isAway = false;
                        //计算30分钟之后闹钟继续闹
                        AlarmTime = new TimeAway(1800000, 1000);// 构造CountDownTimer对象
    ...
    private void DisConnectState() {
        registerView.setTopBarLeftText(((HomeDeviceFragment) registerView).getActivity().getString(R.string.home_disconnectting));
        registerView.setBatteryInfoDisplay(((HomeDeviceFragment) registerView).getActivity().getString(R.string.home_battery), "", R.color.them_bg);
